...
generate_doc() {
    local gendoc_command="go run github.com/hechain20/hechain/common/metrics/cmd/gendoc"
    local orderer_prom
    ...
    local orderer_deps=()
    while IFS= read -r pkg; do orderer_deps+=("$pkg"); done < <(go list -deps github.com/hechain20/hechain/cmd/orderer | sort -u | grep hyperledger)
    ...
    local peer_deps=()
    while IFS= read -r pkg; do peer_deps+=("$pkg"); done < <(go list -deps github.com/hechain20/hechain/cmd/peer | sort -u | grep hyperledger)
