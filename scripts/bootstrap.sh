...
pullBinaries() {
    ...
    download "${BINARY_FILE}" "https://github.com/hechain20/hechain/releases/download/v${VERSION}/${BINARY_FILE}"
    if [ $? -eq 22 ]; then
        echo
...
if [ "$BINARIES" == "true" ]; then
    echo
    echo "Pull Hechain binaries"
    echo
    pullBinaries
fi
if [ "$DOCKER" == "true" ]; then
    echo
    echo "Pull Hechain docker images"
    echo
    pullDockerImages
fi
