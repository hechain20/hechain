...
packages_diff() {
    git -C "${base_dir}" diff --no-commit-id --name-only -r "${1:-HEAD}" |
        (grep '.go$' || true) | \
        sed 's%/[^/]*$%%' | sort -u | \
        awk '{print "github.com/hechain20/hechain/"$1}'
}
...
main() {
    ...
    local -a package_spec=("${TEST_PKGS:-github.com/hechain20/hechain/...}")
