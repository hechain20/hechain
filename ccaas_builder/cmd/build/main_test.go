...
func TestArguements(t *testing.T) {
	...
	// Build ledger binary
	gt := NewWithT(t)
	buildCmd, err := gexec.Build("github.com/hechain20/hechain/ccaas_builder/cmd/build")
...
func TestGoodPath(t *testing.T) {
	...
	releaseCmd, err := gexec.Build("github.com/hechain20/hechain/ccaas_builder/cmd/build")
	gt.Expect(err).NotTo(HaveOccurred())
	defer gexec.CleanupBuildArtifacts()
