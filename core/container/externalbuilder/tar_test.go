	...
	Describe("Untar", func() {
		...
		Context("when the archive contains absolute paths", func() {
			It("returns an error", func() {
				...
				defer file.Close()
				err = externalbuilder.Untar(file, dst)
				Expect(err).To(MatchError("tar contains the absolute or escaping path '/home/yellickj/go/src/github.com/hechain20/hechain/core/chaincode/externalbuilders/testdata/a/test.file'"))
			})
		})
