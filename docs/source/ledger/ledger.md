...
A **ledger** is a key concept in Hechain; it stores important factual
...
ordered transactions (credits and debits) that determine it. Hechain
...
Let's now take a closer look at the Hechain ledger structure!


## The Ledger

In Hechain, a ledger consists of two distinct, though related, parts
...
and we'll see later how Hechain provides flexibility in this regard.
...
retrieval of states.  We'll see later that Hechain can be configured
...
the Hechain SDK; they merely invoke a smart contract, and are
...
internal use by Hechain, and is incremented every time the state
...
In LevelDB and CouchDB, we see an important aspect of Hechain -- it
...
In Hechain, each [channel](../channels.html) has a completely
