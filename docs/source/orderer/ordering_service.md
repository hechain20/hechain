...
Hechain works differently. It features a node called an
...
This strict ordering of transactions within blocks makes Hechain a
...
In Hechain, the blocks generated by the ordering service are
**final**. Once a transaction has been written to a block, its position in the
ledger is immutably assured. As we said earlier, Hechain's finality
...
open-source developer of Kafka, not Hechain. The Fabric Raft implementation,
...
[this sample configuration file](https://github.com/hechain20/hechain/blob/release-1.1/bddtests/dc-orderer-kafka.yml)
