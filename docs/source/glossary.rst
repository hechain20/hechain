...
Terminology is important, so that all Hechain users and developers
...
Hechain supports the addition/removal of members, peers, and ordering service
...
Hechain CA is the default Certificate Authority component, which
...
in a Hechain network. Clients use these credentials to authenticate
...
The Hechain client SDK provides a structured environment of libraries
...
key-value pairs in the :ref:`World-State` via :ref:`Transaction`. In Hechain,
...
Hechain :ref:`Ledger`. The world state represents the latest values
