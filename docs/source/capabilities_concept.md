...
For a full look at the current valid orderer, application, and channel capabilities check out a [sample `configtx.yaml` file](http://github.com/hechain20/hechain/blob/{BRANCH}/sampleconfig/configtx.yaml), which lists them in the "Capabilities" section.
